...
public class religionRecycleAdapter extends RecyclerView.Adapter<religionRecycleAdapter.ViewHolder>{


    ArrayList<Book> mItems;

    public religionRecycleAdapter(){
        super();
        mItems = new ArrayList<>();
        for(int i = 0; i<Book.books.size(); i++){
            if(Book.books.get(i).getGenre().toLowerCase().equals("religion".toLowerCase())){
    ...
    @Override
    public religionRecycleAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, final int i) {
        View v = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.home_book_list, viewGroup, false);
        ViewHolder viewHolder = new religionRecycleAdapter.ViewHolder(v);

        return viewHolder;
    }

    @Override
    public void onBindViewHolder(religionRecycleAdapter.ViewHolder holder, int position) {
