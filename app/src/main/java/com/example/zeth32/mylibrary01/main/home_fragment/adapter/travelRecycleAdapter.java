...
public class travelRecycleAdapter extends RecyclerView.Adapter<travelRecycleAdapter.ViewHolder>{


    ArrayList<Book> mItems;

    public travelRecycleAdapter(){
        super();
        mItems = new ArrayList<>();
        for(int i = 0; i<Book.books.size(); i++){
            if(Book.books.get(i).getGenre().toLowerCase().equals("travel".toLowerCase())){
    ...
    @Override
    public travelRecycleAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, final int i) {
        View v = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.home_book_list, viewGroup, false);
        ViewHolder viewHolder = new travelRecycleAdapter.ViewHolder(v);

        return viewHolder;
    }

    @Override
    public void onBindViewHolder(travelRecycleAdapter.ViewHolder holder, int position) {
