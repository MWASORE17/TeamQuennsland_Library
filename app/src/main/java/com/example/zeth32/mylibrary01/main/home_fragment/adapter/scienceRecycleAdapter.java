...
public class scienceRecycleAdapter extends RecyclerView.Adapter<scienceRecycleAdapter.ViewHolder>{


    ArrayList<Book> mItems;

    public scienceRecycleAdapter(){
        super();
        mItems = new ArrayList<>();
        for(int i = 0; i<Book.books.size(); i++){
            if(Book.books.get(i).getGenre().toLowerCase().equals("science".toLowerCase())){
    ...
    @Override
    public scienceRecycleAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, final int i) {
        View v = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.home_book_list, viewGroup, false);
        ViewHolder viewHolder = new scienceRecycleAdapter.ViewHolder(v);

        return viewHolder;
    }

    @Override
    public void onBindViewHolder(scienceRecycleAdapter.ViewHolder holder, int position) {
